...
function cf7mls_wpcf7_editor_panel_form($post) {
  ...
  <h2 class="cf7mls-title-form"><?php echo esc_html( __( 'Progress Bar', 'contact-form-7' ) ); ?></h2>
  <fieldset class="cf7mls-wrap-form">
    ...
    <div id="cf7mls-app">
      <nav class="cf7mls-app-steps nav-tab-wrapper">
        <a href="javascript:void(0)" class="nav-tab " v-for="(step, key) in steps" @click="selectedStep = step" :class="{ 'nav-tab-active': selectedStep == step }">
          <span class="cf7mls-name-step">{{ step.name }}</span>
          <span class="cf7mls-remove-step" @click.prevent.stop="removeStep(step)">×</span>
        </a>
        <a href="javascript:void(0)" class="nav-tab " @click="newStep">+</a>
      </nav>
      <div class="cf7mls-app-step-content">
        <div v-for="(step, key) in steps" :class="{ 'hidden': selectedStep != step, 'active': selectedStep == step }">
            <div class="cf7mls-app-wrap-bt">
								<div class="cf7mls-app-content">
										<p><?php _e( 'Step Name', 'contact-form-7' ) ?></p>
                    <input name="cf7mls_step_name[]" type="text" v-model="steps[key].title">
								</div>
                <div class="cf7mls-app-content" :class="{ 'hide': key == 0 }">
										<p><?php _e( 'Back Button', 'contact-form-7' ) ?></p>
                    <input type="text" v-model="steps[key].back">
                </div>
                <div class="cf7mls-app-content" :class="{ 'hide': (key == steps.length - 1 && steps.length > 1) || false}">
                    <p><?php _e( 'Next Button', 'contact-form-7' ) ?></p>
                    <input type="text" v-model="steps[key].next">
                </div>
            </div>
            <?php
            if(cf7cmlsIsMoanaActivated()) {
                ?>
                <div class="mls_moana_design_btn_wrap"><a href="javascript:void(0)" @click="editWithMoana(key)" class="cf7-start-editing-btn"><i class="_icon"></i><?php _e('Edit With Moana', 'cf7mls'); ?></a></div>
                <?php
            }
            ?>
            <textarea :data-key="key" v-model="steps[key].content" cols="100" rows="24" class="large-text code"></textarea>
        </div>
      </div>
    </div>
  </fieldset>
...
{   
    ...
    if ($load_js_css === true) {
        ...
        wp_register_script('vuejs', CF7MLS_PLUGIN_URL . '/assets/admin/js/vue.min.js');
        ...
        foreach($scan as $k => $v) {
          if($v->type == 'cf7mls_step') {
            if(count($v->values) == 2) {
              ...
              if($numberStep === 1) {
                $steps[] = array('back' => '', 'next' => $v->values[0], 'title' => $v->values[1]);
              }else {
                $steps[] = array('back' => $v->values[0], 'next' => '', 'title' => $v->values[1]); 
							}
							
            } elseif(count($v->values) == 3) {
              $steps[] = array('back' => $v->values[0], 'next' => $v->values[1], 'title' => $v->values[2]);
            }
					}
					
        }
        wp_localize_script( 'cf7mls', 'cf7mls', array(
          'steps' => $steps,
          'cf7mls_app' => null
        ) );
